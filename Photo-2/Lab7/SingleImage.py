...
class SingleImage(object):
    ...
    def rotationMatrix_vanishingPoints(self, vp1, vp2, vp3):
        """
        ...
        """
        f = self.camera.focalLength
        xp = self.camera.principalPoint[0]
        yp = self.camera.principalPoint[1]
        # computing lambdas
        lam1 = float(f / np.sqrt(
            (vp1[:, 0] - xp) ** 2 + (vp1[:, 1] - yp) ** 2 + f ** 2))
        lam2 = float(f / np.sqrt(
            (vp2[:, 0] - xp) ** 2 + (vp2[:, 1] - yp) ** 2 + f ** 2))
        lam3 = float(f / np.sqrt(
            (vp3[:, 0] - xp) ** 2 + (vp3[:, 1] - yp) ** 2 + f ** 2))

        # computing R matrix from image to object space
        R_matrix = (-1 / f) * np.array(
            [[vp1[:, 0] - xp, vp2[:, 0] - xp, vp3[:, 0] - xp]
                ,
             [vp1[:, 1] - yp, vp2[:, 1] - yp, vp3[:, 1] - yp]
                , [-f, -f, -f]])
        R_matrix = np.dot(R_matrix, np.diag(np.array([lam1, lam2, lam3]))).astype(float)
        # update object
        self.rotationMatrix = R_matrix
...
if __name__ == '__main__':
    ...
    one_more1 = Reader.ReadSampleFile(r"x_axis.json")
    one_more2 = Reader.ReadSampleFile(r"y_axis.json")
    one_more3 = Reader.ReadSampleFile(r"z_axis.json")
